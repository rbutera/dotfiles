...
|---|---|---|---|
| Claude Code | `dot_claude/modify_settings.json` | `cleanupPeriodDays`, `enabledPlugins` | `permissions`, `model`, `attribution`, etc. |
...
Edit `dot_claude/modify_settings.json` and add to the `base` JSON. Note: Claude Code currently uses plugins (not raw MCP server config in settings.json), so MCP servers are typically added via project-level `.mcp.json` files or the Claude Code UI. The modify script manages `enabledPlugins`:
