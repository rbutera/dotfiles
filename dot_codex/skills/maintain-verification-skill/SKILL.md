...
**Platform note.** On Codex or another non-Claude runtime, the parallel per-feature source readers below map to `spawn_agent` fan-out, and the project-local skill lives under your runtime's skills location, not `.claude/skills/`. See [`codex-tools.md`](../poteto-mode/references/codex-tools.md).
