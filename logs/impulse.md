...
## 2026-09-04 — role-packet-corpus producer cron (bead a46ec)

Added the Impulse cron `role-packet-corpus` (`dot_config/impulse/jobs.json.tmpl`, nimbus branch): `0 */6 * * *` Europe/London, zsh wrapper that sources `tools.zsh` (for `bd` on PATH) then execs `~/navi/bin/role-packet-corpus.mjs`. The producer builds `~/navi/state/role-packet-corpus.json` (open non-sealed beads + career notes + `~/expedition/*.md` + chronicler topics + MDX reports + STAR stories + pickable roles) which the read-only `/navi/role-packets` dashboard route reads (the container mounts `~/navi/state` read-only and cannot run `bd`). Mirrors the deadline-radar producer pattern. Deployed `~/.config/impulse/jobs.json` updated directly (no 1Password session for `chezmoi apply`); registration proven via `impulse sync-crons` -> `navi/role-packet-corpus -> 0 5,11,17,23 * * * UTC`.


## 2026-09-06 — role-packet-verdict cron (bead workspace-vgql5)

Added the Impulse cron `role-packet-verdict` (`dot_config/impulse/jobs.json.tmpl`, nimbus branch), directly after the `role-packet-corpus` job it depends on: `10 */6 * * *` Europe/London (ten minutes after the corpus build's `0 */6 * * *`, so the corpus JSON is fresh when the judge reads it), zsh wrapper that sources `tools.zsh` then execs `~/navi/bin/role-packet-verdict.mjs`, `timeout` 600, enabled. The verdict producer reads `~/navi/state/role-packet-corpus.json`, compiles a packet per roster JD with the dashboard's pure matcher, asks the free Codex judge (`openai-codex/gpt-5.6-sol`, Rule 54 free path, no paid key) for FIT / STRETCH / NOT_A_FIT, and writes `~/navi/state/role-packet-verdicts.json` keyed by JD sha256. Idempotent: a JD already carrying a verdict is skipped unless `--force`, so the unattended run only judges new or changed JDs. The dashboard mounts `~/navi/state` read-only and attaches the cached verdict per JD; nothing re-ran the script before, so a new roster role showed "not computed" until run by hand. Ran once by hand to confirm unattended shape: exit 0, one new role judged (Lupa FIT, 7.2s), five cached skipped, file rewritten. Registration in Hatchet is via `impulse sync-crons`, which the worker (`com.rai.impulse-worker`) runs on start; a `chezmoi apply` of the deployed `~/.config/impulse/jobs.json` needs a 1Password session (see below). A `role-packet-verdicts-freshness` guard in `recap-health-monitor.mjs` alarms amber if the verdicts file is more than 24h older than the corpus file. See `~/expedition/Recap Health Monitor Guard Ledger.md`.
