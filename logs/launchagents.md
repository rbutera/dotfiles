...
## 2026-09-04 -- NEW: com.rai.dev-reaper (leaked dev-process reaper)

- **Context**: nimbus hit **load average 72** with **36 GB of 37 GB swap consumed** and ~350 MB free RAM. Diagnosis split two ways. (a) Six concurrent repo gates across six `rennet` agent worktrees, each vitest worker 300-680 MB -- legitimate work, not a leak. (b) **Eight leaked `nx mcp` server chains, four processes deep each** (`npm exec nx mcp` -> `node .bin/nx mcp` -> `pnpm dlx nx-mcp@latest` -> dlx worker), oldest **4d21h**, four owned by ChatGPT.app's codex app-server and four by a codex app-server under harness-bridge. Every one still had a **live parent**, so no orphan-based cleanup would ever have caught them: the host app outlives the conversation and never tears the server down. Killed the four stale ChatGPT.app chains by hand (16 processes); load fell 72 -> 23.7.
- **New agent** `com.rai.dev-reaper`, script `bin/executable_dev-reaper` -> `~/bin/dev-reaper`. `StartInterval` 600s, `RunAtLoad false` (matches `io.focused.worktree-janitor`; no work at login). `ProcessType Background` + `Nice 5` + `LowPriorityIO` -- a job that exists to reduce load must not add to it.
- **Pure zsh + /bin + /usr/bin. No node, no npm.** This is deliberate and is the direct lesson of the 2026-08-21 entry below: every node-invoking plist is coupled to an asdf version string and breaks on each toolchain bump. This one has no such coupling and needs no maintenance across node upgrades. Verified under launchd via `launchctl kickstart -p`: **exit code 0, empty stderr** (i.e. not the exit-127 failure that hit 5 agents on 2026-08-20).
- **Three rules**, allowlist-based (`REAPABLE_RE` = vitest/jest/esbuild/`.bin/{vite,nx,biome,eslint,tsc}`/nx-mcp), with `NEVER_RE` subtracted first (`.app/Contents`, Electron, claude, codex, tokenmaxx, paseo, tsserver, T3 Code, and dev-reaper itself):
  1. `orphan:reparented` -- candidate with ppid 1. No allowlisted pattern is ever a launchd job, so ppid 1 means the real parent died.
  2. `orphan:worktree-gone` / `orphan:cwd-deleted` -- candidate whose argv names a `*/worktrees/<x>` that no longer exists, or whose cwd has been deleted underneath it. **macOS lsof reports the original path for a deleted cwd with no "(deleted)" marker**, so the check is `lsof -d cwd` then existence-test the result.
  3. `mcp-chain:stale` -- `nx mcp` chains older than `MCP_MAX_AGE_HOURS` (4), always keeping the newest `MCP_KEEP_NEWEST` (2). This is the only rule that kills something with a live parent, and it is the only rule that would have prevented this incident. Reaps the `npm exec nx mcp` wrapper at the top so the whole 4-process chain goes.
- TERM, 10s grace, then KILL. Dry run with `DEV_REAPER_DRY_RUN=1`.
- **Testing note, since the first version of the self-test lied.** It copied `/bin/sleep` to a name containing "vitest" for the fixture. Copying a signed macOS system binary into /tmp invalidates its signature, the kernel SIGKILLs it on exec, **both fixtures died instantly, and the test printed `PASS: orphan was reaped` while the reaper's own log said `nothing to reap`** -- a pass satisfied by a process that never ran. The fixture is now a shell script, and the test carries (a) a liveness precondition, (b) a precondition that the fixture actually matches the candidate scan, and (c) an assertion that the reaper **logged** `orphan:cwd-deleted` for that pid. "The process is gone" is not evidence the reaper did anything. Run it with `~/bin/dev-reaper --selftest`; it asserts both that an orphan IS reaped and that a live sibling is NOT.
- Rule 3's kill path was separately control-proven by forcing `MCP_MAX_AGE_HOURS=0 MCP_KEEP_NEWEST=0` under dry-run and confirming it targets the `npm exec nx mcp` wrapper of all four live chains.
- **Not implemented on purpose**: a swap-pressure valve that kills the oldest idle daemons during heavy builds. Considered and declined -- it can kill something still in use, and the concurrent gates are real work, not a leak.
